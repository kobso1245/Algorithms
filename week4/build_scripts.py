...
def topo_sort(graph_table, curr_node, poseten_e_moje_bi_a_moje_bi_ne, v_putq_e, result):

    poseten_e_moje_bi_a_moje_bi_ne[curr_node] = 1
    v_putq_e[curr_node] = 1

    for naslednik in graph_table[curr_node]:
        if  poseten_e_moje_bi_a_moje_bi_ne[naslednik] == 0 and  v_putq_e[naslednik] == 0:
            v_putq_e[naslednik] = 1
            poseten_e_moje_bi_a_moje_bi_ne[naslednik] = 1
            res = topo_sort(graph_table, naslednik, poseten_e_moje_bi_a_moje_bi_ne, v_putq_e, result)
            if  not res:
                return False
...
def start():
    ...
    result = []
    res = topo_sort(table, start_node, poseten_e_moje_bi_a_moje_bi_ne, v_putq_e, result)
    try:
        if res:
