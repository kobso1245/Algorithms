...
def decode(input_text):
    ...
    # converting the message so we can get the key, alphabet and the encoded
    # message itself
    ...
    indexes_of_coded_message = convert_from_message_to_numbers(
        message, alphabet)
    # converting the key to numbers
    full_key = key * \
        (len(message) // int(splitted[2])) + key[:(len(message) % int(splitted[2]))]
    ...
    for i in range(length_message):
        indexes_of_decoded_message.append(
            indexes_of_coded_message[i] - indexes_of_key[i])
    # turning the decoded message to message with letters
    original_message = from_indexes_to_message(
        indexes_of_decoded_message, alphabet)
