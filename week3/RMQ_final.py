class RMQ:
        ...
        if end_index - start_index <= 1:
            if self.length - 1 < pos:
                for i in range(self.length, pos + 1):
                    ...
                    self.length += 1
            self.arr[pos] = (
                min(arr[start_index], arr[end_index]), (start_index, end_index))
            return self.arr[pos][0]
        self.arr[pos] = (min(self.__builder(arr,
                                            start_index,
    ...
    def __add(self, index, value, pos, curr_start, curr_end):
        if (curr_end - curr_start == 1 and (curr_end == index or curr_start ==
                                            index)) or (curr_end == curr_start and index == curr_end):
        ...
        if index <= left_node_end:
            self.arr[pos] = (min(self.__add(index,
                                            value,
                                            2 * pos + 1,
                                            curr_start,
                                            left_node_end),
