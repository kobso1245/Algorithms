...
def binary_search(sorted_array, elem_to_find):
    ...
    while(left <= right):
        if curr_elem>0 and sorted_array[curr_elem] == elem_to_find and sorted_array[curr_elem - 1] < elem_to_find:
            return curr_elem
        if sorted_array[curr_elem] < elem_to_find:
            left = curr_elem + 1
        if sorted_array[curr_elem] >= elem_to_find:
...
def main_testing_function():
    ...
    if linear_search(sorted_array, random_elem) == binary_search(sorted_array, random_elem):
        return True
    return False
